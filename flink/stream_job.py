"""
PyFlink streaming job skeleton for the coding challenge.

Complete `build_pipeline` so that it:

  1. Consumes the Debezium JSON from Kafka topic `orders_cdc`
  2. Parses the envelope and extracts:
        order_id, customer_id, quantity, price, event_ts
  3. Calculates per‑customer 1‑minute tumbling window aggregations:
        - total gross_sales = quantity * price
        - order_count
  4. Writes aggregations to Postgres table `public.order_agg`
     using primary key (window_start, customer_id)
"""
